...
- The data measures tooth length in guinea pigs using 3 dose levels of Vitamin C
...
Click on the `pigs.csv` and choose `Import Dataset` **then pause when you see the following**
...
`pigs <- read_csv(file = "data/pigs.csv")`
...
```{r, eval=FALSE}
pigs <- read_csv("data/pigs.csv", comment = "#")
```
